...
def average_perceptron(x_train, y_train, x_valid, y_valid, iters):
    ...
    if s + c > 0:
        w_avg=(s*w_avg+c*w)/(s+c)
    return w_avg, train_err, valid_err

def average_perceptron2(x_train, y_train, x_valid, y_valid, iters):
    train_err = np.zeros(iters)
    valid_err = np.zeros(iters)
    w = np.zeros(x_train.shape[1])
    w_avg = np.zeros(x_train.shape[1])
...
w, train_err, valid_err = average_perceptron(x_train, y_train, x_valid, y_valid, 150)   # Accordingly, the best epoch is 7
...
plt.ylabel('Error %')
plt.show()
w, train_err, valid_err = average_perceptron(x_train, y_train, x_valid, y_valid, 7)
y_pred=predict(w,x_test)
np.savetxt("aplabel.csv", y_pred, delimiter=",",fmt='%i')
